...
IMAGE_SIZE = (256, 256)
UNCERTAINTY_THRESHOLD = 0.55  # unter 45% => Modell ist unsicher
...
with ui.column().classes('w-full items-center'):
    ...
    ui.label('Lade ein Foto eines Autos hoch oder nimm mit dem Handy direkt ein Foto auf.')
    ...
    with ui.card().classes('w-full max-w-2xl p-6'):
        ...
        async def handle_upload(e):
            try:
                ...
                if best_conf < UNCERTAINTY_THRESHOLD:
                    top1_label.set_text(
                        f"Unsicher: Das Modell ist sich nicht sicher. Beste Vermutung: "
                        f"{pretty_brand_name(best['brand'])} ({best_conf * 100:.2f}%)"
                    )
                    confidence_label.set_text(
                        f"Hinweis: Unter {UNCERTAINTY_THRESHOLD * 100:.0f}% wird die Vorhersage als unsicher markiert."
                    )
                else:
                    top1_label.set_text(
        ...
        ui.label(
            'Auf dem Handy öffnet sich je nach Browser direkt die Kamera oder eine Auswahl zwischen Kamera und Galerie.'
